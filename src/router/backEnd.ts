...
export async function initBackEndControlRoutes() {
	...
	// 获取路由菜单数据
	const { data: { result } } = await getBackEndControlRoutes();
	// 存储接口原始路由（未处理component），根据需求选择使用
	store.dispatch('requestOldRoutes/setBackEndControlRoutes', JSON.parse(JSON.stringify(result)));
	// 处理路由（component），替换 dynamicRoutes（/@/router/route）第一个顶级 children 的路由
	dynamicRoutes[0].children = await backEndComponent(result);
	// 添加动态路由
	await setAddRoute();
